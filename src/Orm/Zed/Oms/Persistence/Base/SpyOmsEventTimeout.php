 ...
 */
abstract class SpyOmsEventTimeout implements ActiveRecordInterface
{
    /**
     * TableMap class name
     */
    const TABLE_MAP = '\\Orm\\Zed\\Oms\\Persistence\\Map\\SpyOmsEventTimeoutTableMap';
    ...
    /**
     * Initializes internal state of Orm\Zed\Oms\Persistence\Base\SpyOmsEventTimeout object.
     */
     ...
     * @return $this|\Orm\Zed\Oms\Persistence\SpyOmsEventTimeout The current object (for fluent API support)
     */
    public function setIdOmsEventTimeout($v)
    {
        ...
        if (($this->isNew() && $hasDefaultValue) || $this->id_oms_event_timeout !== $v) {
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : SpyOmsEventTimeoutTableMap::translateFieldName('FkOmsOrderItemState', TableMap::TYPE_PHPNAME, $indexType)];
            $this->fk_oms_order_item_state = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : SpyOmsEventTimeoutTableMap::translateFieldName('FkSalesOrderItem', TableMap::TYPE_PHPNAME, $indexType)];
            $this->fk_sales_order_item = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : SpyOmsEventTimeoutTableMap::translateFieldName('Event', TableMap::TYPE_PHPNAME, $indexType)];
            $this->event = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : SpyOmsEventTimeoutTableMap::translateFieldName('Timeout', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\Orm\\Zed\\Oms\\Persistence\\SpyOmsEventTimeout'), 0, $e);
        }
    }
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_FIELDNAME.
     * @return $this|\Orm\Zed\Oms\Persistence\SpyOmsEventTimeout
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\Orm\Zed\Oms\Persistence\SpyOmsEventTimeout The current object, for fluid interface
     */
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Orm\Zed\Oms\Persistence\SpyOmsEventTimeout (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Orm\Zed\Oms\Persistence\SpyOmsEventTimeout Clone of current object.
     * @throws PropelException
     */
