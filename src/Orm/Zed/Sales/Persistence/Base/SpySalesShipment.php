...
abstract class SpySalesShipment implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Orm\\Zed\\Sales\\Persistence\\Map\\SpySalesShipmentTableMap';
     ...
     * @return $this|\Orm\Zed\Sales\Persistence\SpySalesShipment The current object (for fluent API support)
     */
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : SpySalesShipmentTableMap::translateFieldName('CarrierName', TableMap::TYPE_PHPNAME, $indexType)];
            $this->carrier_name = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : SpySalesShipmentTableMap::translateFieldName('DeliveryTime', TableMap::TYPE_PHPNAME, $indexType)];
            $this->delivery_time = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 6 + $startcol : SpySalesShipmentTableMap::translateFieldName('Name', TableMap::TYPE_PHPNAME, $indexType)];
            $this->name = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 7 + $startcol : SpySalesShipmentTableMap::translateFieldName('RequestedDeliveryDate', TableMap::TYPE_PHPNAME, $indexType)];
            $this->requested_delivery_date = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 8 + $startcol : SpySalesShipmentTableMap::translateFieldName('CreatedAt', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            $this->created_at = (null !== $col) ? PropelDateTime::newInstance($col, null, 'DateTime') : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 9 + $startcol : SpySalesShipmentTableMap::translateFieldName('UpdatedAt', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            ...
            return $startcol + 10; // 10 = SpySalesShipmentTableMap::NUM_HYDRATE_COLUMNS.
    ...
    public function toArray($keyType = TableMap::TYPE_FIELDNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        $result = array(
            ...
            $keys[4] => $this->getCarrierName(),
            $keys[5] => $this->getDeliveryTime(),
            $keys[6] => $this->getName(),
            $keys[7] => $this->getRequestedDeliveryDate(),
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_FIELDNAME.
     * @return $this|\Orm\Zed\Sales\Persistence\SpySalesShipment
     */
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_FIELDNAME)
    {
        ...
        if (array_key_exists($keys[4], $arr)) {
            $this->setCarrierName($arr[$keys[4]]);
        }
        if (array_key_exists($keys[5], $arr)) {
            $this->setDeliveryTime($arr[$keys[5]]);
        }
        if (array_key_exists($keys[6], $arr)) {
            $this->setName($arr[$keys[6]]);
        }
        if (array_key_exists($keys[7], $arr)) {
            $this->setRequestedDeliveryDate($arr[$keys[7]]);
        }
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\Orm\Zed\Sales\Persistence\SpySalesShipment The current object, for fluid interface
     */
     ...
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildSpySalesShipment The current object (for fluent API support)
     */
