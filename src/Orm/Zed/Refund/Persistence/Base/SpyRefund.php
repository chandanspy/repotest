 ...
 *
 *
 * @package    propel.generator.src.Orm.Zed.Refund.Persistence.Base
 */
abstract class SpyRefund implements ActiveRecordInterface
{
    /**
     * TableMap class name
     */
    const TABLE_MAP = '\\Orm\\Zed\\Refund\\Persistence\\Map\\SpyRefundTableMap';
    ...
    /**
     * Initializes internal state of Orm\Zed\Refund\Persistence\Base\SpyRefund object.
     */
     ...
     * @return $this|\Orm\Zed\Refund\Persistence\SpyRefund The current object (for fluent API support)
     */
    public function setIdRefund($v)
    {
        ...
        if (($this->isNew() && $hasDefaultValue) || $this->id_refund !== $v) {
            $this->id_refund = $v;
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : SpyRefundTableMap::translateFieldName('Comment', TableMap::TYPE_PHPNAME, $indexType)];
            $this->comment = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : SpyRefundTableMap::translateFieldName('CreatedAt', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 5; // 5 = SpyRefundTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\Orm\\Zed\\Refund\\Persistence\\SpyRefund'), 0, $e);
        }
    }
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_FIELDNAME.
     * @return $this|\Orm\Zed\Refund\Persistence\SpyRefund
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\Orm\Zed\Refund\Persistence\SpyRefund The current object, for fluid interface
     */
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Orm\Zed\Refund\Persistence\SpyRefund (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Orm\Zed\Refund\Persistence\SpyRefund Clone of current object.
     * @throws PropelException
     */
