 ...
 *
 *
 * @package    propel.generator.src.Orm.Zed.GlossaryStorage.Persistence.Base
 */
abstract class SpyGlossaryStorage implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Orm\\Zed\\GlossaryStorage\\Persistence\\Map\\SpyGlossaryStorageTableMap';
     ...
     * @return $this|\Orm\Zed\GlossaryStorage\Persistence\SpyGlossaryStorage The current object (for fluent API support)
     */
    ...
    public function setGlossaryKey($v)
    {
        ...
        if (($this->isNew() && $hasDefaultValue) || $this->glossary_key !== $v) {
            $this->glossary_key = $v;
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : SpyGlossaryStorageTableMap::translateFieldName('Data', TableMap::TYPE_PHPNAME, $indexType)];
            $this->data = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : SpyGlossaryStorageTableMap::translateFieldName('GlossaryKey', TableMap::TYPE_PHPNAME, $indexType)];
            $this->glossary_key = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : SpyGlossaryStorageTableMap::translateFieldName('Key', TableMap::TYPE_PHPNAME, $indexType)];
            $this->key = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : SpyGlossaryStorageTableMap::translateFieldName('Locale', TableMap::TYPE_PHPNAME, $indexType)];
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_FIELDNAME.
     * @return $this|\Orm\Zed\GlossaryStorage\Persistence\SpyGlossaryStorage
     */
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_FIELDNAME)
    {
        ...
        if (array_key_exists($keys[3], $arr)) {
            $this->setGlossaryKey($arr[$keys[3]]);
        }
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\Orm\Zed\GlossaryStorage\Persistence\SpyGlossaryStorage The current object, for fluid interface
     */
     ...
     *
     * @return     $this|ChildSpyGlossaryStorage The current object (for fluent API support)
     */
