...
abstract class SpyTouch implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Orm\\Zed\\Touch\\Persistence\\Map\\SpyTouchTableMap';
     ...
     * @return $this|\Orm\Zed\Touch\Persistence\SpyTouch The current object (for fluent API support)
     */
    public function setIdTouch($v)
    {
        ...
        if (($this->isNew() && $hasDefaultValue) || $this->id_touch !== $v) {
            $this->id_touch = $v;
    ...
    public function setItemId($v)
    {
        ...
        if (($this->isNew() && $hasDefaultValue) || $this->item_id !== $v) {
            $this->item_id = $v;
    ...
    public function setItemType($v)
    {
        ...
        if (($this->isNew() && $hasDefaultValue) || $this->item_type !== $v) {
            $this->item_type = $v;
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : SpyTouchTableMap::translateFieldName('ItemEvent', TableMap::TYPE_PHPNAME, $indexType)];
            $this->item_event = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : SpyTouchTableMap::translateFieldName('ItemId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->item_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : SpyTouchTableMap::translateFieldName('ItemType', TableMap::TYPE_PHPNAME, $indexType)];
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_FIELDNAME.
     * @return $this|\Orm\Zed\Touch\Persistence\SpyTouch
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\Orm\Zed\Touch\Persistence\SpyTouch The current object, for fluid interface
     */
