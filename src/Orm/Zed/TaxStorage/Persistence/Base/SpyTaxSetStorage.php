 ...
 */
abstract class SpyTaxSetStorage implements ActiveRecordInterface
{
    /**
     * TableMap class name
     */
    ...
    public function setIdTaxSetStorage($v)
    {
        ...
        if (($this->isNew() && $hasDefaultValue) || $this->id_tax_set_storage !== $v) {
    ...
    public function setAliasKeys($v)
    {
        ...
        if (($this->isNew() && $hasDefaultValue) || $this->alias_keys !== $v) {
            $this->alias_keys = $v;
            $this->modifiedColumns[SpyTaxSetStorageTableMap::COL_ALIAS_KEYS] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : SpyTaxSetStorageTableMap::translateFieldName('AliasKeys', TableMap::TYPE_PHPNAME, $indexType)];
            $this->alias_keys = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : SpyTaxSetStorageTableMap::translateFieldName('Key', TableMap::TYPE_PHPNAME, $indexType)];
            $this->key = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : SpyTaxSetStorageTableMap::translateFieldName('CreatedAt', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            $this->created_at = (null !== $col) ? PropelDateTime::newInstance($col, null, 'DateTime') : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 6 + $startcol : SpyTaxSetStorageTableMap::translateFieldName('UpdatedAt', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            ...
            return $startcol + 7; // 7 = SpyTaxSetStorageTableMap::NUM_HYDRATE_COLUMNS.
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(SpyTaxSetStorageTableMap::COL_ALIAS_KEYS)) {
        ...
        try {
            $pk = $con->lastInsertId('id_tax_set_storage_pk_seq');
        } catch (Exception $e) {
            throw new PropelException('Unable to get autoincrement id.', 0, $e);
        }
        $this->setIdTaxSetStorage($pk);
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(SpyTaxSetStorageTableMap::COL_ALIAS_KEYS)) {
            $criteria->add(SpyTaxSetStorageTableMap::COL_ALIAS_KEYS, $this->alias_keys);
        }
        if ($this->isColumnModified(SpyTaxSetStorageTableMap::COL_KEY)) {
